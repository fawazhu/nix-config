...
in {
  home.packages = if enable then [ pkgs.codeium ] else [];
  programs.neovim.plugins = if enable then [ pkgs.vimPlugins.codeium-vim ] else [];

  xdg.configFile."nvim/after/syntax/codeium-syntax.vim" = if enable then { text = ''
    highlight CodeiumSuggestion guifg=#808080 ctermfg=244
  '';} else {};
  xdg.configFile."nvim/after/plugin/codeium.lua" = if enable then { text = ''
